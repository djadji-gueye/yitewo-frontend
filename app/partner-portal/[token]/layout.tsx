...
export default function PartnerPortalLayout({ children }: { children: React.ReactNode }) {
  ...
  // Ferme le menu à chaque nav (mobile)
  ...
  if (error || !partner) return (
    <div style={{ minHeight: "100vh", background: "#fafaf8", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "DM Sans, sans-serif", padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #f0ebe8", padding: "48px 36px", textAlign: "center", maxWidth: 400 }}>
        ...
        <p style={{ color: "#6b6b6b", fontSize: 14, lineHeight: 1.6, marginBottom: 24 }}>{error || "Ce lien partenaire est invalide ou a expiré."}</p>
    ...
    <>
      <style>{`
        ...
        @media (max-width: 768px) {
          .portal-sidebar { transform: translateX(-100%); transition: transform 0.3s ease; z-index: 40; }
          .portal-sidebar.open { transform: translateX(0); box-shadow: 4px 0 24px rgba(0,0,0,0.15); }
          ...
          .portal-topbar { padding: 0 12px !important; }
          .portal-content { padding: 16px !important; }
          .portal-boutique-label { display: none }
        }
      `}</style>
      ...
      <div style={{ display: "flex", minHeight: "100vh", background: "#f7f4f2", fontFamily: "DM Sans, sans-serif" }}>
        ...
        <aside className={`portal-sidebar${menuOpen ? " open" : ""}`}
          ...
          <div style={{ padding: "24px 20px 20px", borderBottom: "1px solid #f0ebe8" }}>
            <div style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: 20, color: "#1a1a1a", marginBottom: 10 }}>yite<span style={{ color: "#E8380D" }}>wo</span></div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 38, height: 38, borderRadius: 10, background: "linear-gradient(135deg, #E8380D22, #E8380D44)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>
                {partner.type === "Restaurant" ? "🍽️" : "🛒"}
              </div>
              <div style={{ minWidth: 0 }}>
                <p style={{ fontWeight: 700, fontSize: 13, color: "#1a1a1a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{partner.name}</p>
        ...
        {/* Main */}
        <div className="portal-main" style={{ flex: 1, marginLeft: 240, display: "flex", flexDirection: "column", minWidth: 0 }}>

          <header className="portal-topbar"
            style={{ height: 56, background: "#fff", borderBottom: "1px solid #f0ebe8", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px", flexShrink: 0, position: "sticky", top: 0, zIndex: 30 }}>
            ...
            <a href={`/order?partner=${partner.slug}`} target="_blank" rel="noopener noreferrer"
              ...
              <span className="portal-boutique-label">Voir ma boutique</span>
              <span style={{ display: "none" }} className="portal-boutique-icon">↗</span>
            </a>
          </header>

          <main className="portal-content" style={{ flex: 1, padding: "28px", overflowY: "auto", minWidth: 0 }}>
