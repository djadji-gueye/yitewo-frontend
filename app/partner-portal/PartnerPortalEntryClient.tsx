...
export default function PartnerPortalEntryClient() {
  const router = useRouter();
  const [noToken, setNoToken] = useState(false);

  useEffect(() => {
    const last = typeof window !== "undefined" ? localStorage.getItem("yitewo_last_partner_token") : null;
    if (last) {
      router.replace(`/partner-portal/${last}`);
    } else {
      setNoToken(true);
    }
  }, [router]);
    ...
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12, padding: 24, textAlign: "center", fontFamily: "DM Sans, sans-serif", background: "#fafaf8" }}>
        ...
        <p style={{ color: "#666", fontSize: 14, maxWidth: 320 }}>
          Ouvre le lien personnalisé reçu par WhatsApp ou SMS pour accéder à ton espace partenaire.
        </p>
      </div>
    );
  }
