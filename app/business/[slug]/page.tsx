    ...
    <Suspense fallback={<div style={{ minHeight: "100vh", padding: 40 }}>Chargement…</div>}>
      <main style={{ minHeight: "100vh", background: "var(--surface)", padding: "48px 20px" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", background: "#fff", borderRadius: 20, padding: 28, border: "1px solid var(--border)" }}>
          <span style={{ display: "inline-block", padding: "5px 10px", borderRadius: 99, background: business.status === "VERIFIED" ? "#d1fae5" : "#fff7ed", color: business.status === "VERIFIED" ? "#047857" : "#c2410c", fontSize: 12, fontWeight: 700 }}>
            {business.status === "VERIFIED" ? "✓ Commerce vérifié" : "Commerce référencé"}
          </span>
          <h1 style={{ fontFamily: "Syne, sans-serif", fontSize: 32, margin: "18px 0 8px" }}>{business.name}</h1>
          <p style={{ color: "var(--muted)", marginBottom: 20 }}>📍 {business.address || business.zone || business.city}</p>
          ...
          <p style={{ color: "var(--muted)", lineHeight: 1.6, marginBottom: 24 }}>
            Si vous êtes le propriétaire de cet établissement, demandez la validation de cette fiche pour la gérer sur Yitewo.
          </p>
          <a href={`/business/${encodeURIComponent(business.slug)}/claim`} style={{ display: "inline-block", borderRadius: 10, padding: "12px 18px", background: "var(--brand)", color: "#fff", fontWeight: 700, textDecoration: "none" }}>
